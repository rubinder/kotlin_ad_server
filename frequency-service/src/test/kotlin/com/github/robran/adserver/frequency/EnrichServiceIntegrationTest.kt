...
class EnrichServiceIntegrationTest {
    ...
    @Test
    fun `returns empty response for unknown user`() =
        runTest {
            val resp =
                stub.enrichForAuction(
    ...
    @Test
    fun `returns counts for campaigns with stored counters`() =
        runTest {
    ...
    @Test
    fun `returns recent categories from winhistory zset`() =
        runTest {
            redisClient.zadd(
                "winhistory:user-A",
                "c1:IAB13" to 1000.0,
    ...
    @Test
    fun `combined response - counts AND categories in one RPC`() =
        runTest {
    ...
    @Test
    fun `empty user_id is rejected with INVALID_ARGUMENT`() =
        runTest {
            val ex =
                runCatching {
    ...
    @Test
    fun `enrichForAuction records redis_lookup_duration with op tags`() =
        runTest {
            ...
            val testServer =
                io.grpc.inprocess.InProcessServerBuilder.forName(testServerName)
                    .directExecutor()
                    .addService(recordingService)
                    .build()
                    .start()
            val testChannel =
                io.grpc.inprocess.InProcessChannelBuilder.forName(testServerName)
                    .directExecutor()
                    .build()
